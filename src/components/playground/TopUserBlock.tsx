...
export default function TopUserBlock({playerData , waiting, showBubbleChat, myTurn} : TopUserBlockProps) {
    ...
    for (let i = 0; i < 14; i++) {
      hiddenCardsBlock.push(
        <Image key={i} src={HiddenCard} alt="Logo Image" priority className="pt-0 origin-left w-[14px] md:w-[25px]  between-lg-and-2xl:w-[30px] h-auto" />
      );
    }

    
    return (
      <div className="user-3 flex gap-5 md:gap-10 ">
        <div className="pick-card-block rotate-180 w-max">
          <PickCard isAnyPlayerWaiting={waiting} />
        </div>
        <div className="user-block flex flex-col gap-3 md:gap-[19px]">
