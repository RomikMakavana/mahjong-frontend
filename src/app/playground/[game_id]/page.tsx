...
export default function GameLayout() {
  ...
  const [openExitGameComfirmationModal, setOpenExitGameConfirmationGameModal] =
    useState(false);
  ...
  useEffect(() => {
    ...
    const userGamePublicRef = ref(
      AuthService.database,
      "user-games/" + gameId + "/public"
    );
    ...
    const dbRef = ref(
      AuthService.database,
      "user-games/" + gameId + "/user-details/" + user?.firebaseUser.uid
    );
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(dbRef, (snapshot) => {
      ...
      if (
        !snapshotData ||
        snapshotData.is_game_started != gameData.is_game_started ||
        snapshotData.status != gameData.status
      ) {
        loadGameDetails();
      } else {
        ...
        if (Array.isArray(snapshotPlayers) && snapshotPlayers.length > 0) {
          ...
          for (const key in _player_in_sequence) {
            const player = snapshotPlayers.find(
              (value) => value._id == _player_in_sequence[key]._id
            );
    ...
    <div>
      {!(gameData !== null) ? (
        ...
        <Fragment>
                ...
                <div className="header flex text-white justify-between mt-6 mb-4 gap-3  items-center">
                  <div className="logo">
                    <Image
                      ...
                      alt="Logo Image"
                      priority
                      className="w-[65px]   between-md-and-sm:w-[125px] h-auto"
                    />
                  </div>
                        ...
                        {otherDetails.map((value, index) => {
                          return (
                            <button
                              ...
                              key={index}
                              className="flex items-center py-3 group/item px-5 last:mb-0 w-full transition-all duration-300 hover:bg-brand-blue hover:bg-opacity-30"
                            >
                  ...
                  <div className="playground flex flex-col h-full justify-between mx-2 md:ml-5 md:mr-5  py-4 md:pb-7 md:pt-7">
                    <div className="flex justify-between another-user-block h-[100%]">
                      ...
                      {gameData.player_in_sequence.length > 3 ? (
                        <LeftUserBlock
                          myTurn={
                            gameData.player_in_sequence[3]._id ==
                            activePlayerId()
                          }
                          showBubbleChat={false}
                          playerData={
                            gamePlayersData[gameData.player_in_sequence[3]._id]
                          }
                    ...
                    {gameData.player_in_sequence.length > 0 ? (
                      <BottomUserBlock
                        ...
                        myTurn={
                          gameData.player_in_sequence[0]._id == activePlayerId()
                        }
                        showBubbleChat={false}
                        gameData={gameData}
                        playerData={
                          gamePlayersData[gameData.player_in_sequence[0]._id]
                        }
          ...
          {gameData &&
            gameData.is_game_started === false &&
            gameData.game_code && openMatchStartedModal && (
              <MatchCreated
                open={openMatchStartedModal}
                closeModal={setOpenMatchStartedModal}
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-5">
            <button
              ...
              className="flex-1 text-white py-3.5 text-sm sm:text-base font-bold opacity-60 border border-white border-opacity-10 rounded-9 max-xs:text-sm"
            >
              Exit
            </button>
            <button
              onClick={() => tournOnFullScreenMode()}
              className="flex-1 bg-brand-blue py-3.5 text-base font-bold text-white rounded-9 max-xs:text-sm"
            >
              Continue
            </button>
          </div>
      ...
      <MahjongModel open={openInformationModal} extraCss="w-[400px]">
        <div className="">
          <h2 className="text-2xl font-bold text-white">
            Please rotate your phone for batter user experience.
          </h2>
        </div>
      </MahjongModel>
    </div>
